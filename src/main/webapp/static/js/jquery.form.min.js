...
(d=d.offset(),c.clk_x=a.pageX-d.left,c.clk_y=a.pageY-d.top):(c.clk_x=a.pageX-h.offsetLeft,c.clk_y=a.pageY-h.offsetTop));setTimeout(function(){c.clk=c.clk_x=c.clk_y=null},100)}function r(){if(b.fn.ajaxSubmit.debug){var a="[jquery.form] "+Array.prototype.join.call(arguments,"");window.console&&window.console.log?window.console.log(a):window.opera&&window.opera.postError&&window.opera.postError(a)}}var z,B;z=void 0!==b("\x3cinput type\x3d'file'/\x3e").get(0).files;B=void 0!==window.FormData;var D=!!b.fn.prop;
...
c," (",c.name,")"),h(z),u&&clearTimeout(u),u=void 0}}var c=n.attr2("target"),k=n.attr2("action"),m=n.attr("enctype")||n.attr("encoding")||"multipart/form-data";l.setAttribute("target",s);g&&!/post/i.test(g)||l.setAttribute("method","POST");k!=e.url&&l.setAttribute("action",e.url);e.skipEncodingOverride||g&&!/post/i.test(g)||n.attr({encoding:"multipart/form-data",enctype:"multipart/form-data"});e.timeout&&(u=setTimeout(function(){y=!0;h(A)},e.timeout));var p=[];try{if(e.extraData)for(var q in e.extraData)e.extraData.hasOwnProperty(q)&&
...
b.isXMLDoc(q);r("isXml\x3d"+f);if(!f&&window.opera&&(null===q.body||!q.body.innerHTML)&&--C){r("requeing onLoad callback, DOM not available");setTimeout(h,250);return}var g=q.body?q.body:q.documentElement;k.responseText=g?g.innerHTML:null;k.responseXML=q.XMLDocument?q.XMLDocument:q;f&&(e.dataType="xml");k.getResponseHeader=function(a){return{"content-type":e.dataType}[a.toLowerCase()]};g&&(k.status=Number(g.getAttribute("status"))||k.status,k.statusText=g.getAttribute("statusText")||k.statusText);
...
(k.responseXML=H(k.responseText));try{B=I(k,l,e)}catch(G){a="parsererror",k.error=c=G||a}}catch(E){r("error caught: ",E),a="error",k.error=c=E||a}k.aborted&&(r("upload aborted"),a=null);k.status&&(a=200<=k.status&&300>k.status||304===k.status?"success":"error");"success"===a?(e.success&&e.success.call(e.context,B,"success",k),x.resolve(k.responseText,"success",k),t&&b.event.trigger("ajaxSuccess",[k,e])):a&&(void 0===c&&(c=k.statusText),e.error&&e.error.call(e.context,k,a,c),x.reject(k,"error",c),
...
l.clk)&&(m=c.name)&&!c.disabled&&(e.extraData=e.extraData||{},e.extraData[m]=c.value,"image"==c.type&&(e.extraData[m+".x"]=l.clk_x,e.extraData[m+".y"]=l.clk_y));var A=1,z=2;c=b("meta[name\x3dcsrf-token]").attr("content");(m=b("meta[name\x3dcsrf-param]").attr("content"))&&c&&(e.extraData=e.extraData||{},e.extraData[m]=c);e.forceSync?f():setTimeout(f,10);var B,q,C=50,F,H=b.parseXML||function(a,b){window.ActiveXObject?(b=new ActiveXObject("Microsoft.XMLDOM"),b.async="false",b.loadXML(a)):b=(new DOMParser).parseFromString(a,
"text/xml");return b&&b.documentElement&&"parsererror"!=b.documentElement.nodeName?b:null},J=b.parseJSON||function(a){return window.eval("("+a+")")},I=function(a,c,e){var d=a.getResponseHeader("content-type")||"",f="xml"===c||!c&&0<=d.indexOf("xml");a=f?a.responseXML:a.responseText;f&&"parsererror"===a.documentElement.nodeName&&b.error&&b.error("parsererror");e&&e.dataFilter&&(a=e.dataFilter(a,c));"string"===typeof a&&("json"===c||!c&&0<=d.indexOf("json")?a=J(a):("script"===c||!c&&0<=d.indexOf("javascript"))&&
